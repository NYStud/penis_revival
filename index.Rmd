...
## ggplot theme
theme_set(theme_readthedown() + theme(plot.caption = element_text(hjust = 0)))
...
cols <- RColorBrewer::brewer.pal(length(unique(penis$Region)), name = "Paired")

plot_ly(data = penis, x = ~length_flaccid, y = ~length_erect) %>%
  add_markers(color = ~Region, hoverinfo = "text",
              text = text, colors = cols) %>%
  layout(xaxis = list(title = "Flaccid Length (cm)"), 
