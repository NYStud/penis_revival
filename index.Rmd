...
Let's look at the N of the individual datapoints (because every row in our dataset represents the data of a specific study). We'll exclude rows with an N over 200, because that's probably a suffienctly large sample anyway.
...
For the remainder of the analysis, we'll be excluding any data point based on an N smaller than 50, because that's the arbitrary judgement of precision I just came up with.
  ...
  mutate(order_length = mean(length)) %>%
  {
    ...
    scale_y_continuous(breaks = seq(0, 100, 2), 
                       ...
                       sec.axis = sec_axis(trans = ~./2.54, 
                                           labels = label_in)) +
    labs(title = "World Penis Data", subtitle = "Length by State",
         x = "Region", y = "Length", fill = "State", color = "State",
         caption = plot_caption) +
